  ...
  server.registerTool(
    'lock_update',
    {
      ...
      description:
        ...
        'ACTIVE locks only: the done archive is an audit record and is not rewritable in place. If an archived lock genuinely needs to change, lock_reopen it first.',
      inputSchema: {
  ...
  server.registerTool(
    'lock_reap',
    {
      ...
      description:
        ...
        'If lock_id is given but that lock is NOT actually stale, this errors rather than reaping it. A supplied stale_minutes may only LENGTHEN the window; it is floored at the CONFIGURED default, so the parameter cannot shorten the way to a live lock. The floor is that configured default and not a constant, so AGENT_LOCKS_STALE_MINUTES set small DOES let reap finish live locks — the environment variable is the operator-level escape, the parameter is the part that is closed. ' +
